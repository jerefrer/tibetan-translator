    ...
    computed: {
      text () {
        if (this.exportType == 'translation')
          return this.lines.map('translation').join('\n');
        else if (this.exportType == 'alternated')
          return this.lines.map('source').zip(this.lines.map('translation')).flatten().join('\n');
      }
    }
  }
</script>
      ...
      <v-card>
        ...
        <v-card-text class="pt-1">
          ...
          >
            ...
            <v-radio value="alternated" label="Tibetan alternated with translation" />
          </v-radio-group>
