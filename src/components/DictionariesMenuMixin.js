...
export default {
  ...
  data () {
    return {
      dictionaries: SqlDatabase.dictionaries.map((dictionary) => {
  ...
  computed: {
    dictionariesForCurrentResults () {
      return this.dictionaries.filter((dictionary) => {
        return this.entries?.find((entry) => entry.dictionaryId == dictionary.id);
      });
    },
    enabledDictionariesIds () {
      return this.dictionariesForCurrentResults.filter((dictionary) => dictionary.enabled).map('id');
    },
    entriesForEnabledDictionaries () {
      if (this.entries == undefined)
        return [];
      return this.entries.filter(
        (entry) => this.enabledDictionariesIds.includes(entry.dictionaryId)
      )
    },
  },
  methods: {
