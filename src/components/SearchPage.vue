...
import { phoneticsStrictFor, phoneticsLooseFor, replaceTibetanGroups, syllablesFor } from '../utils'
...
export default {
  ...
  watch: {
    '$route.params.query'(value) {
      ...
      if (value)
        this.performSearch({ query: value, fromNavigation: true });
    },
  ...
  computed: {
    ...
    sortedEntries() {
      return this.entriesForEnabledDictionaries.sortBy((entry) => {
        var positions = [];
        this.regularSearchTerms.each((term) => {
          positions.push((entry.term + entry.definition).indexOf(term));
        })
        this.phoneticsStrictSearchTerms.each((phoneticsStrictTerm) => {
          positions.push((entry.termPhoneticsStrict + entry.definitionPhoneticsWordsStrict).indexOf(phoneticsStrictTerm));
        })
        this.phoneticsLooseSearchTerms.each((phoneticsLooseTerm) => {
          positions.push((entry.termPhoneticsLoose + entry.definitionPhoneticsWordsLoose).indexOf(phoneticsLooseTerm));
        })
        return [
          positions.sum(),                              // [  9 ] < [ 10 ] => false
    ...
    limitedEntries() {
      return this.sortedEntries.
        from((this.resultsPage - 1) * this.numberOfEntriesPerPage).
        to(this.numberOfEntriesPerPage)
    },
    ...
    searchTerms() {
      if (!this.searchQuery)
        ...
        return this.substituteWylieTerms(this.searchQuery).
          split('&').map('trim').compact(true);
    },
    regularSearchTerms() {
    ...
    phoneticsStrictSearchTerms() {
      ...
      return terms.map(
        (term) => PhoneticSearch.prepareTermForStrictMatching(term)
      )
    },
    phoneticsLooseSearchTerms() {
      var terms = this.phoneticsTerms(/^\{.*\}$/, phoneticsLooseFor);
      return terms.map(
        (term) => PhoneticSearch.prepareTermForLooseMatching(term)
      )
    },
  },
  methods: {
    substituteWylieTerms(text) {
      return text.replace(/\(([^)]*)\)/g, (wylieWithParenthesis) => {
        return wylieToUnicode.convert(wylieWithParenthesis.from(1).to(-1))
      })
    },
    phoneticsTerms(regexp, convert) {
      return this.searchTerms.
        ...
        flatten().compact().
        map((term) => term.from(1).to(-1)).
        map((term) => replaceTibetanGroups(term, (tibetan) => convert(tibetan) + ' '));
    },
    ...
    highlightSearchTerms(definition) {
      ...
      this.phoneticsStrictSearchTerms.each((term) => {
        highlighted = this.highlightTibetanMatchingPhonetics(highlighted, term, phoneticsStrictFor);
      });
      this.phoneticsLooseSearchTerms.each((term) => {
        highlighted = this.highlightTibetanMatchingPhonetics(highlighted, term, phoneticsLooseFor);
      });
      return highlighted;
    },
    highlightTibetanMatchingPhonetics(definition, term, convert) {
      return replaceTibetanGroups(definition, (group) => {
        ...
        var combinations = this.everySyllablesCombinationsOfGivenLengthFor(group, numberOfSyllablesForTerm);
    ...
    cleanUpEmFromLinks(text) {
      return text.replace(/href="[^\"]*"/g, (match) => match.replace(/<\/?em>/g, ''));
    },
    everySyllablesCombinationsOfGivenLengthFor(source, numberOfSyllablesForTerm) {
    ...
    storeQuery() {
      this.previousQueries.remove(this.searchQuery);
      this.previousQueries.unshift(this.searchQuery);
      if (this.previousQueries.length > this.maxNumberOfPreviousQueries)
        this.previousQueries = this.previousQueries.first(this.maxNumberOfPreviousQueries);
    },
    pushRoute() {
    ...
    performSearch(options = {}) {
      ...
      if (this.searchQuery?.trim()) {
        ...
        var params = [];
        this.regularSearchTerms.each((term) => {
          conditions.push(`(entries_fts MATCH 'term:${this.prepareTerm(term)} OR definition:${this.prepareTerm(term)}')`);
        })
        this.phoneticsStrictSearchTerms.each((phoneticsStrictTerm) => {
          conditions.push(`(entries_fts MATCH 'termPhoneticsStrict:${this.prepareTerm(phoneticsStrictTerm)} OR definitionPhoneticsWordsStrict:${this.prepareTerm(phoneticsStrictTerm)}')`);
        })
        this.phoneticsLooseSearchTerms.each((phoneticsLooseTerm) => {
          conditions.push(`(entries_fts MATCH 'termPhoneticsLoose:${this.prepareTerm(phoneticsLooseTerm)} OR definitionPhoneticsWordsLoose:${this.prepareTerm(phoneticsLooseTerm)}')`);
        })
        var query = `
    ...
    shortDictionaryLabelForHasTibetan(entry) {
      return this.shortDictionaryLabelFor(entry).match(TibetanRegExps.tibetanGroups);
    }
  },
  mounted() {
    this.setPageTitle();
...
<template>
  <div class="search-page">
    <v-system-bar app height="63">
      <v-text-field autofocus clearable hide-details height="63" ref="input" class="flex-grow-1 text-center tibetan"
        :value="searchQuery" @keyup.enter="performSearch()" @click:clear="clear">
        <template v-slot:prepend-inner>
          &nbsp;
          <ResultsAndPaginationAndDictionaries ref="resultsAndPaginationAndDictionaries" v-if="entries != undefined"
            :loading="paginationLoading" :page="resultsPage" :dictionaries="dictionariesForCurrentResults"
            :numberOfEntriesPerPage="numberOfEntriesPerPage"
            :totalNumberOfEntries="totalNumberOfEntriesForEnabledDictionaries" @change:page="changePage($event)"
            @close:dictionariesMenu="focusInput" />
        </template>
        <template v-slot:append>
          <v-progress-circular size="24" color="info" indeterminate :style="{
                opacity: loading ? 1 : 0
              }" />
          <v-btn text tile x-large height="60" color="primary" :disabled="!searchQuery || !searchQuery.trim()"
            @click="performSearch()">
            <v-icon left>
              mdi-magnify
            </v-icon>
    ...
    <v-fade-transition mode="out-in" appear>
      ...
      <template v-if="!loading">
        ...
        <template v-if="entries == undefined">
          ...
          <div v-if="previousQueries.length > 0" class="previous-queries grey--text">
  
            <div class="header">
              <v-btn fab x-small class="mr-2 grey--text" elevation="0" @click="clearPreviousQueries">
            ...
            <v-fade-transition group tag="div" class="buttons">
              <v-btn text v-for="query in previousQueries" class="previous-query" :key="query" color="grey"
                @click="performSearch({ query: query })" v-html="wrapAllTibetanWithSpansAndAddTshekIfMissing(query)" />
            </v-fade-transition>
        ...
        <v-simple-table v-else-if="limitedAndDecoratedEntries.length" key="entries">
          <tbody>
            <tr v-for="entry in limitedAndDecoratedEntries" class="entry">
              <td>
                <v-row>
                  ...
                  <v-col cols="12" sm="2">
                    <v-chip label small class="ml-2 px-2" color="dictionary-label grey darken-2"
                      v-html="shortDictionaryLabelFor(entry)" :class="{
...
.search-page .v-input .v-input__slot .v-input__prepend-inner .results-and-pagination {}
...
.search-page .v-data-table>.v-data-table__wrapper>table>tbody>tr:hover:not(.v-data-table__expanded__content):not(.v-data-table__empty-wrapper) {
