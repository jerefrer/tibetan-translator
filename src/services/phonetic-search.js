import 'sugar'
import '../../vendor/javascripts/normalize'

export default {
  prepareTermForStrictMatching (term) {
    return this.splitSyllables(term)
      .split(' ').map((subterm) =>
        subterm.
        toLowerCase().
        replace(/a'?i/g, 'e').
  ...
  prepareTermForLooseMatching (term) {
    return this.splitSyllables(term)
      .split(' ').map((subterm) =>
        subterm.
        normalize(). // will also take care of ö=>o and ü=>u
        toLowerCase().
        replace(/a'?i/g, 'e').
  ...
  splitSyllables(text) {
    ...
    var result = "";
    while(text) {
      var match = text.match(/((?:siddhi)|(?:ut)|((?:dz)|(?:st)|(?:sv)|(?:ng)|(?:tn)|(?:ts)|(?:[dtjn]r)|(?:[tzsclkd]h)|(?:[kgntd][h]?y)|[jwpbmntdkglrcszyvwh])?[']?[aeiouöüéè][rmnkhpbl]?[g]?[']?[i]?)$/);
      if (match) {
        result = match[1] + ' ' + result.trim();
        text = text.to(match.index).trim()
      } else { // If the text is not valid phonetics we just return it as-is
